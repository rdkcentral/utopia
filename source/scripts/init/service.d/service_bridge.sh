...
cmdiag_ebtables_rules()
{
    if [ "$1" = "enable" ] ; then
        ...
        CMDIAG_MAC_BRLAN0="`cat /sys/class/net/brlan0/address`"
        ...
        ebtables -A BRIDGE_FORWARD_FILTER -s "$CMDIAG_MAC" -o eth3 -j DROP
        ebtables -A BRIDGE_FORWARD_FILTER -s "$CMDIAG_MAC_BRLAN0" -o eth3 -j DROP
...
cmdiag_if()
{
	...
	CMDIAG_MAC="`cat /sys/class/net/lan0/address`"
        ...
        ovs-vsctl  add-port brlan0 l"${CMDIAG_IF}"
    else
        ifconfig "$CMDIAG_IF" down
        ifconfig l"${CMDIAG_IF}" down
        ip link del "$CMDIAG_IF"
        ovs-vsctl del-port brlan0 l"${CMDIAG_IF}"
...
forward_wan_lan_traffic()
{
    ...
    if [ "$1" = "enable" ] ; then
        # set up veth interface to forward brlan0 and erouter traffic in bridge mode
            ...
            ip link add lbr0 type veth peer name ler0
            ifconfig lbr0 up 
            ifconfig ler0 up
	    echo 1 > /proc/sys/net/ipv6/conf/ler0/disable_ipv6
            echo 1 > /proc/sys/net/ipv6/conf/lbr0/disable_ipv6
	    brctl addif erouter0 ler0
            MAX_WAIT_TIME=120
            TRIES=0
	    ovs-vsctl  add-port brlan0 lbr0
            while [  "$TRIES" -lt "$MAX_WAIT_TIME" ] ; do

                   if [ "`sysevent get multinet_$INSTANCE-status`" = "ready" ];then
                        # check_iface_exists_in_bridge=`brctl show brlan0  | grep lbr0`
                        check_iface_exists_in_bridge=`ovs-vsctl list-ports brlan0 | grep lbr0`
                        if [ -z "$check_iface_exists_in_bridge" ];then
                            echo_t "multinet_$INSTANCE-status status is ready...,adding lbr0 to brlan0 and breaking the loop"
			    ovs-vsctl  add-port brlan0 lbr0 
	    ...
	    ovs-vsctl del-port brlan0 lbr0
