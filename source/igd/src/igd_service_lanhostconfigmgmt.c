...
LOCAL INT32 _igd_service_LANHostConfigMgmt_destroy(IN struct upnp_service *pservice)
{
	...
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","Destroy LANDevice LANHostConfigManagement");
	if(pservice==NULL)
		return -1;
...
{	
	...
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","Initilize LANHostConfigManagement of LANDevice %d",
	...
	{
		RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.IGD","strdup() out of memory,type!\n");
		_igd_service_LANHostConfigMgmt_destroy(LANHostConfigMgmt_service);
		return NULL;
	}
	...
	{
		RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.IGD","create LANHostConfigManagement description file fail!\n");
		_igd_service_LANHostConfigMgmt_destroy(LANHostConfigMgmt_service);
		return NULL;
	}
...
LOCAL INT32 _igd_get_DHCPServerConfigurable (INOUT struct action_event *event)
{
	...
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","GetDHCPServerConfigurable of LAN%d\n",local_index.lan_device_index);
...
LOCAL INT32 _igd_get_DHCPRelay (INOUT struct action_event *event)
{
	...
	local_index = *((struct device_and_service_index*)event->service->private);
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","GetDHCPRelay of LAN%d\n",local_index.lan_device_index);
...
LOCAL INT32 _igd_get_SubnetMask (INOUT struct action_event *event)
{
	...
	local_index = *((struct device_and_service_index*)event->service->private);
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","GetSubnetMask of LAN%d\n",local_index.lan_device_index);
...
#define RANGE_ADDR_BUF_SZ 32
	...
	local_index = *((struct device_and_service_index*)event->service->private);
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","GetAddressRange of LAN%d\n",local_index.lan_device_index);
...
LOCAL INT32 _igd_get_IPRoutersList (INOUT struct action_event *event)
{
	...
	local_index = *((struct device_and_service_index*)event->service->private);
	RDK_LOG(RDK_LOG_INFO, "LOG.RDK.IGD","GetIPRoutersList of LAN%d\n",local_index.lan_device_index);
